...
class theme_adaptable_core_renderer extends core_renderer {
        ...
        foreach ($menu->get_children() as $item) {
            ...
            $content .= $this->render_from_template('theme_adaptable/activitygroups', $context);
        }
    ...
    /**
     * TAKEN FROM THE BOOST THEME core_renderer.php
    ...
    /**
     * TAKEN FROM THE FORDSON THEME core_renderer.php
    **/
    public function teacherdash() {
        ...
        if (isloggedin() && ISSET($COURSE->id) && $COURSE->id > 1) {
            ...
            if (has_capability('moodle/course:viewhiddenactivities', $context)) {
                $togglebutton = get_string('coursemanagementbutton', 'theme_adaptable');
            } else {
                $togglebuttonstudent = get_string('studentdashbutton', 'theme_adaptable');
            }
        }
        $course = $this->page->course;
        $context = context_course::instance($course->id);
        ...
        $userlinks = get_string('userlinks', 'theme_adaptable');
        $userlinksdesc = get_string('userlinks_desc', 'theme_adaptable');
        $qbank = get_string('qbank', 'theme_adaptable');
        $qbankdesc = get_string('qbank_desc', 'theme_adaptable');
        $badges = get_string('badges', 'theme_adaptable');
        $badgesdesc = get_string('badges_desc', 'theme_adaptable');
        $coursemanage = get_string('coursemanage', 'theme_adaptable');
        $coursemanagedesc = get_string('coursemanage_desc', 'theme_adaptable');
        $coursemanagementmessage = (empty($PAGE->theme->settings->coursemanagementtextbox)) ? false : format_text($PAGE->theme->settings->coursemanagementtextbox);
        ...
        //Student Dash
            ...
            $hascourseinfogroup = array (
                'title' => get_string('courseinfo', 'theme_adaptable'),
                'icon' => 'map'
            );
            $summary = theme_adaptable_strip_html_tags($COURSE->summary);
            $summarytrim = theme_adaptable_course_trim_char($summary, 300);
            $courseinfo = array (
                array(
            ...
            $hascoursestaff = array (
                'title' => get_string('coursestaff', 'theme_adaptable'),
                'icon' => 'users'
            );
            ...
            $activitylinkstitle = get_string('activitylinkstitle', 'theme_adaptable');
            $activitylinkstitle_desc = get_string('activitylinkstitle_desc', 'theme_adaptable');
            $mygradestext = get_string('mygradestext', 'theme_adaptable');
            $myprogresstext = get_string('myprogresstext', 'theme_adaptable');
        ...
        if ($globalhaseasyenrollment && $coursehaseasyenrollment) {
            ...
            return $this->render_from_template('theme_adaptable/teacherdash', $dashlinks );
        
    }

     /**
     * Override of the moodle function course_modchooser
     */
    public function course_modchooser($modules, $course) {
        if (!$this->page->requires->should_create_one_time_item_now('core_course_modchooser')) {
            return '';
        }
        $modchooser = new \adaptable\output\modchooser($course, $modules);
